...
--
TableUtil.contact = function(t1,...)
	local argCount = select('#',...);
	if argCount > 0 then
		for index=1,select('#',...) do
			local argValue = select(index,...);
			if type(argValue) == 'table' then
				for k,v in pairs(argValue) do
					table.insert(t1,#t1+1,tostring(v));
				end	
			else
				table.insert(t1,#t1+1,tostring(argValue));
			end	
		end
	else
		Logger.print("arg length is 0 at TableUtil.contact");
	end
    return t1;
end
...
function unserializePHP(data, offset)
	offset = offset or 0
	--[[
...
function print_f(data)
	...
	local function serialize2(obj, num)
	    local lua = ""
	    local t = type(obj)
	    if t == "number" then
	        lua = lua .. obj
	    ...
	    elseif t == "table" then
		    ...
		    for k, v in pairs(obj) do
		    	local tnum = num + 1
		    	lua = lua .. string.rep(space, num)
		        lua = lua .. "[" .. serialize2(k, tnum) .. "]=" .. serialize2(v, tnum) .. ",\n"
		    end

		    if tag then
		    	lua = lua .. string.rep(space, num-1) .. "}"
		    else
		    	lua = lua .. "}"
		    end
	    ...
	    elseif t == "nil" then
	        ...
	        error("can not serialize2 a " .. t .. " type.")
	    end

	    return lua
	end
	
	print(serialize2(data, 1))
end
